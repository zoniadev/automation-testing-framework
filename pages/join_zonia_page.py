...
class JoinZoniaPage(BasePage):
    ...
    def join_zonia(self):
        ...
        if self.context.docuseries_prefix == 'pc':
            ...
            next_url = getattr(common_variables, f'{self.context.funnel}_sign_up_url')
        else:
    ...
    def join_zonia_screening(self, direct):
        ...
        if direct:
            ...
            self.wait_for_navigation(getattr(common_variables, f'{self.context.funnel}_sign_up_url'),
                                     timeout=20000)
        else:
            ...
            self.wait_for_navigation(getattr(common_variables, f'{self.context.funnel}_join_zonia_url'), timeout=20000)
    ...
    def join_zonia_replay_weekend(self):
        ...
        self.wait_for_navigation(getattr(common_variables, f'{self.context.funnel}_rw_sales_url'), timeout=20000)
        self.context.page.locator(JOIN_ZONIA_ID_BUTTON).click()
    ...
    def join_zonia_episode(self):
        ...
        if self.context.funnel == 'cr_bonus':
            next_page = getattr(common_variables, f'{self.context.funnel}_rw_signup_url')
        elif self.context.funnel == 'cr_live':
            next_page = getattr(common_variables, f'{self.context.funnel}_sign_up_url')
        elif self.context.funnel == 'cr_1ep':
            next_page = getattr(common_variables, f'{self.context.funnel}_join_zonia_url')
        else:
            raise Exception(f'Unsupported parameter {self.context.funnel}!')
